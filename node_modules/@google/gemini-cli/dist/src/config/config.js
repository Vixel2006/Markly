...
export async function parseArguments(settings) {
    const yargsInstance = yargs(hideBin(process.argv))
        ...
        .command('$0 [promptWords...]', 'Launch Gemini CLI', (yargsInstance) => yargsInstance
        .option('model', {
        alias: 'm',
        type: 'string',
        description: `Model`,
        ...
        .deprecateOption('show-memory-usage', 'Use the "ui.showMemoryUsage" setting in settings.json instead. This flag will be removed in a future version.')
        .deprecateOption('sandbox-image', 'Use the "tools.sandbox" setting in settings.json instead. This flag will be removed in a future version.')
        ...
        if (argv['prompt'] && promptWords && promptWords.length > 0) {
            throw new Error('Cannot use both a positional prompt and the --prompt (-p) flag together');
        }
...
export async function loadHierarchicalGeminiMemory(currentWorkingDirectory, includeDirectoriesToReadGemini = [], debugMode, fileService, settings, extensionContextFilePaths = [], folderTrust, memoryImportFormat = 'tree', fileFilteringOptions) {
