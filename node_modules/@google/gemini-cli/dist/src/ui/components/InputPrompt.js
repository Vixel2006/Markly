...
export const InputPrompt = ({ buffer, onSubmit, userMessages, onClearScreen, config, slashCommands, commandContext, placeholder = '  Type your message or @path/to/file', focus = true, inputWidth, suggestionsWidth, shellModeActive, setShellModeActive, approvalMode, onEscapePromptChange, vimHandleInput, isShellFocused, }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Box, { borderStyle: "round", borderColor: statusColor ?? (focus ? theme.border.focused : theme.border.default), paddingX: 1, children: [_jsxs(Text, { color: statusColor ?? theme.text.accent, "aria-label": statusText || undefined, children: [shellModeActive ? (reverseSearchActive ? (_jsxs(Text, { color: theme.text.link, "aria-label": SCREEN_READER_USER_PREFIX, children: ["(r:)", ' '] })) : ('!')) : commandSearchActive ? (_jsx(Text, { color: theme.text.accent, children: "(r:) " })) : showYoloStyling ? ('*') : ('>'), ' '] }), _jsx(Box, { flexGrow: 1, flexDirection: "column", children: buffer.text.length === 0 && placeholder ? (focus ? (_jsxs(Text, { children: [chalk.inverse(placeholder.slice(0, 1)), _jsx(Text, { color: theme.text.secondary, children: placeholder.slice(1) })] })) : (_jsx(Text, { color: theme.text.secondary, children: placeholder }))) : (linesToRender
                            ...
                            tokens.forEach((token, tokenIdx) => {
                                ...
                                if (isOnCursorLine) {
                                    ...
                                    if (relativeVisualColForHighlight >= tokenStart &&
                                        relativeVisualColForHighlight < tokenEnd) {
                                        const charToHighlight = cpSlice(token.text, relativeVisualColForHighlight - tokenStart, relativeVisualColForHighlight - tokenStart + 1);
                                        const highlighted = chalk.inverse(charToHighlight);
                                        display =
                                            cpSlice(token.text, 0, relativeVisualColForHighlight - tokenStart) +
                                                highlighted +
                                                cpSlice(token.text, relativeVisualColForHighlight - tokenStart + 1);
                                    }
                                    charCount = tokenEnd;
                                }
                                const color = token.type === 'command' || token.type === 'file'
                                    ? theme.text.accent
                                    : theme.text.primary;
                                renderedLine.push(_jsx(Text, { color: color, children: display }, `token-${tokenIdx}`));
                            });
                            const currentLineGhost = isOnCursorLine ? inlineGhost : '';
                            if (isOnCursorLine &&
                                cursorVisualColAbsolute === cpLen(lineText)) {
                                if (!currentLineGhost) {
                                    renderedLine.push(_jsx(Text, { children: chalk.inverse(' ') }, `cursor-end-${cursorVisualColAbsolute}`));
                                }
                            }
                            const showCursorBeforeGhost = focus &&
                                ...
                                cursorVisualColAbsolute === cpLen(lineText) &&
                                currentLineGhost;
                            return (_jsx(Box, { height: 1, children: _jsxs(Text, { children: [renderedLine, showCursorBeforeGhost && chalk.inverse(' '), currentLineGhost && (_jsx(Text, { color: theme.text.secondary, children: currentLineGhost }))] }) }, `line-${visualIdxInRenderedSet}`));
                        })
