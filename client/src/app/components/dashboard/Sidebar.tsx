...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: Bookmark,
      panel: "bookmarks",
      path: "/app/bookmarks/all",
    },
    {
      name: "Favorites",
      icon: Star,
      panel: "favorites",
      path: "/app/bookmarks/favorite",
    },
    {
      name: "AI Suggested",
      icon: Lightbulb,
      panel: "ai-suggestions",
      path: "/app/ai-suggestions",
    },
      ...
      <div className="flex-1 p-4 custom-scrollbar">
        ...
        <nav className="space-y-2 mb-6">
          {navItems.map((item) => (
            ...
            >
              <item.icon
                className={`w-5 h-5 flex-shrink-0 ${activePanel === item.panel ? "text-indigo-700" : "text-slate-600"}`}
              />
              {isExpanded && (
                <motion.span
        ...
        <div className="space-y-2">
          {collections.map((collection) => (
            ...
            >
              <motion.span
                className={`flex items-center gap-3 overflow-hidden ${!isExpanded && "justify-center w-full"}`}
                variants={itemTextVariants}
              >
                <Folder
                  className={`w-5 h-5 flex-shrink-0 ${selectedCollectionId === collection.id ? "text-indigo-700" : "text-slate-600"}`}
                />
                {isExpanded && (
